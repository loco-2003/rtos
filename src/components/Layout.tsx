...
import { LayoutDashboard, MonitorPlay, ShieldAlert, FileText, Cpu } from 'lucide-react';
...
export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-screen bg-[#F5F5F5] text-[#141414] font-sans overflow-hidden">
      {/* Sidebar */}
      <aside className="w-64 border-r border-black/5 bg-white flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <NavItem to="/monitor" icon={<Cpu size={18} />} label="System Monitor" />
          <NavItem to="/exam" icon={<FileText size={18} />} label="Student Client" />
          <NavItem to="/proctor" icon={<ShieldAlert size={18} />} label="Proctoring Unit" />
        </nav>
